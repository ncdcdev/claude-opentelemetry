...
| `OTEL_METRIC_EXPORT_INTERVAL` | `60000` | メトリクス送信間隔（ミリ秒）。`10000`（10秒）から延長するとデータポイント数を削減できる |
| `OTEL_LOGS_EXPORT_INTERVAL` | `5000` | ログ送信間隔（ミリ秒） |
| `OTEL_LOG_TOOL_DETAILS` | `0` | ツール呼び出しの詳細ログ（`1` で有効、`0` で無効。無効化するとLogs容量を大幅削減） |
| `OTEL_METRICS_INCLUDE_SESSION_ID` | `false` | メトリクスに `session_id` ラベルを付与するか（`false` でカーディナリティ削減） |
...
| `session_id` | CLI セッションの ID（`OTEL_METRICS_INCLUDE_SESSION_ID=false` で除外可能） |
| `model` | 使用したモデル名 |

これらのラベルを使い、Grafana ダッシュボードでユーザー別の集計・絞り込みが可能です。

> **データ量の最適化**: `session_id` はダッシュボードのフィルタリングに使用されていないため、`OTEL_METRICS_INCLUDE_SESSION_ID=false` を設定してもモニタリング機能に影響はありません。Grafana Cloud Free Tier の無料枠に収めるには、この設定に加えて `OTEL_METRIC_EXPORT_INTERVAL` を延長（例: `60000`）し、`OTEL_LOG_TOOL_DETAILS=0` でツール詳細ログを無効化することを推奨します。
